...
// Config is the application's decoded JSON configuration. Legacy UDR fields
// remain at the top level for backward compatibility; Providers contains the
// canonical multi-provider schema.
type Config struct {
    ...
    // Providers is the canonical multi-provider configuration. It is mutually
    // exclusive with the legacy User, Password, and Domains fields.
...
// apiResponse is the parsed envelope returned by the UDR command API.
...
// dnsRecord contains the normalized fields needed to compare and replace a
// record while retaining its original wire representation for deletion.
...
func (config Config) validate() error {
    if len(config.Providers) > 0 {
        if strings.TrimSpace(config.User) != "" || strings.TrimSpace(config.Password) != "" || len(config.Domains) > 0 {
            return errors.New("providers cannot be combined with legacy user, password, or domains fields")
        }
        if _, err := config.interval(); err != nil {
            return err
        }

        seenProviders := make(map[string]struct{}, len(config.Providers))
        for i, provider := range config.Providers {
            ...
            if _, exists := seenProviders[name]; exists {
                return fmt.Errorf("provider name %q is configured more than once", provider.Name)
            }
    ...
    return validateDomains(config.Domains, "domains")
}

// validateDomains validates UDR zone ownership and rejects duplicate zones or
// hostnames. path is included in errors for both legacy and provider schemas.
func validateDomains(domains []DomainConfig, path string) error {
    if len(domains) == 0 {
        return fmt.Errorf("%s must contain at least one entry", path)
    }
    ...
    for i, domain := range domains {
        ...
        if len(domain.Subdomains) == 0 {
            return fmt.Errorf("%s[%d].subdomains must contain at least one entry", path, i)
        }
        ...
        for j, subdomain := range domain.Subdomains {
            ...
            if subdomainName == "" {
                return fmt.Errorf("%s[%d].subdomains[%d] is empty", path, i, j)
            }
...
func (u updater) run(ctx context.Context, config Config, pollInterval time.Duration) error {
    ...
    for _, provider := range providers {
        provider.logConfigured(ctx)
    }

    var observedIPv4 string
    var observedIPv6 string
    ...
    for {
        ...
        if err != nil {
            ...
            if ipv4 != observedIPv4 || ipv6 != observedIPv6 {
                u.logf(
                    "External IP address change detected: IPv4 %s -> %s, IPv6 %s -> %s",
                    displayIPAddress(observedIPv4),
                    displayIPAddress(ipv4),
                    displayIPAddress(observedIPv6),
            ...
            for _, provider := range providers {
                if err := provider.update(ctx, ipv4, ipv6); err != nil {
                    u.logf("DNS update failed for provider %s: %v", provider.name(), err)
                }
            }
        }
...
// logConfiguredZones records the complete online UDR zone state at startup.
// Query failures are logged per zone and do not prevent the polling loop from
// starting, so they can be retried during the first update.
...
// displayIPAddress provides a readable log value for a missing address family.
...
// queryDNSZoneRecords fetches and parses the currently published records for
// one UDR zone.
...
// buildSubdomainUpdateForm creates a non-destructive UDR update: it deletes
// only A and AAAA records belonging to the managed hostname, then adds the
// available address families. Other record types and hostnames are untouched.
...
// hasRecordChanges reports whether a UDR form contains at least one record
...
// verifySubdomainRecords requires exactly one matching online record for each
// available family, no stale record for an unavailable family, and TTL 600.
...
// ipAddressesEqual compares parsed addresses so equivalent IPv6 spellings are
// treated as equal.
...
// parseDNSRecord extracts the owner, TTL, type, and value from a UDR resource
// record line while preserving the original line for exact deletion requests.
...
// callAPI submits one UDR command, bounds and logs its response with credential
// redaction, and accepts only successful HTTP and provider-level status codes.
...
// responseBodyLimit permits large zone listings while keeping ordinary command
// responses tightly bounded.
...
// recordChange is one ordered addrrN or delrrN parameter used for safe logging.
type recordChange struct {
    key   string
    value string
}

// recordChanges extracts consecutive deletion and addition parameters in the
// order in which the provider applies them.
...
// parseAPIResponse decodes the line-oriented UDR response format. Unrecognized
// lines are ignored, while property arrays are grouped by normalized name.
...
// apiPropertyName extracts the first bracketed name from a UDR property key,
// for example "property[RR][0]" becomes "rr".
